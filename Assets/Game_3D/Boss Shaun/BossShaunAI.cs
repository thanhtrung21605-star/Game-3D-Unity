...
public class BossShaunAI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        anim = phase1Model.GetComponent<Animator>();
        if (anim == null) anim = phase1Model.GetComponentInChildren<Animator>();
    ...
    void Update()
    {
        ...
        // Luôn bật animation bay
        if (anim != null) anim.SetBool("isFlying", !isDead);
    }
    ...
    public void TakeDamage(int damage)
    {
        ...
        if (enemyHP <= 0) 
        {
            if (!isPhase2) StartCoroutine(TransformationRoutine());
            else Die();
        }
        else 
        {
    ...
    {
        if (anim != null) anim.SetTrigger("Transform");
        yield return new WaitForSeconds(2.0f);

        phase1Model.SetActive(false);
        phase2Model.SetActive(true);
        anim = phase2Model.GetComponent<Animator>();
        if (anim == null) anim = phase2Model.GetComponentInChildren<Animator>();
    ...
    IEnumerator AttackRoutine() { Transform p = GetPlayer(); if (p == null) { canAttack = true; yield break; } canAttack = false; Vector3 dirToPlayer = (p.position - transform.position).normalized; transform.rotation = Quaternion.LookRotation(new Vector3(dirToPlayer.x, 0, dirToPlayer.z)); if (anim != null) anim.SetTrigger("Bite"); yield return new WaitForSeconds(0.3f); RaycastHit hit; if (Physics.Raycast(transform.position + Vector3.up * 0.5f, dirToPlayer, out hit, attackRange)) { if (hit.collider.CompareTag("Player")) { hit.collider.SendMessageUpwards("TakeDamage", biteDamage, SendMessageOptions.DontRequireReceiver); } } yield return new WaitForSeconds(attackCooldown - 0.3f); canAttack = true; }
