...
public class CrabAI : MonoBehaviour
{
    ...
    [Header("=== Âm Thanh ===")]
    public AudioClip hitSound;    // Kéo tiếng bị đánh vào đây
    public AudioClip attackSound; // Kéo tiếng cắn/tấn công vào đây
    ...
    public void TakeDamage(int damage)
    {
        ...
        // [MỚI] Phát âm thanh bị đánh
        if (hitSound != null) AudioSource.PlayClipAtPoint(hitSound, transform.position, 1.0f);
        
        enemyHP -= damage;

        if (healthBar != null) healthBar.ShowAndHealth(enemyHP, 3);
    ...
    // Các hàm Coroutine khác giữ nguyên...
    ...
    IEnumerator InvulnerabilityRoutine(float duration) { isInvulnerable = true; yield return new WaitForSeconds(duration); isInvulnerable = false; }
    IEnumerator PlayHitEffectDelayed() { yield return new WaitForSeconds(hitEffectDelay); GameObject effect = Instantiate(hitEffectPrefab, transform.position + Vector3.up * 0.5f, Quaternion.identity); effect.transform.localScale = Vector3.one * hitEffectScale; Destroy(effect, hitEffectDuration); }
    ...
    IEnumerator AttackRoutine() { 
        ...
        // [MỚI] Phát âm thanh tấn công
        if (attackSound != null) AudioSource.PlayClipAtPoint(attackSound, transform.position, 1.0f);
